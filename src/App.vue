...
import DiscoOrb from './components/DiscoOrb.vue'; // 引入 3D 组件
...
.noise-overlay {
  ...
  pointer-events: none; z-index: 5; opacity: var(--noise-opacity);
...
/* 标题样式 */
h1.main-title {
  font-family: 'Cinzel', serif; font-size: 3rem; text-transform: uppercase;
  letter-spacing: 0.2em; color: var(--ink-white);
  margin: 0 0 10px 0; text-align: center; position: relative;
  text-shadow: 2px 2px 4px rgba(0,0,0,0.8), 0 0 10px rgba(255,255,255,0.1);
  border-bottom: 2px solid var(--ink-white); padding-bottom: 10px; width: 100%;
}
h1.main-title::after {
  content: ''; position: absolute; bottom: -6px; left: 0; width: 100%; height: 1px; background: var(--faded-white);
}
.subtitle {
  font-family: 'Space Mono', monospace; font-size: 0.8rem; color: var(--faded-white);
  text-transform: uppercase; letter-spacing: 0.1em; margin-bottom: 40px;
}
...
.terminal-wrapper {
  ...
  box-shadow: 0 0 0 4px rgba(0,0,0,0.5), 0 0 20px rgba(0,0,0,1);
  padding: 2px;
}
.terminal-inner {
...
.input-area {
  display: flex; border-bottom: 1px dashed rgba(255,255,255,0.2);
  padding-bottom: 15px; margin-bottom: 15px;
}
textarea {
  width: 100%; background: transparent; border: none;
  color: var(--terminal-gold); font-family: 'Space Mono', monospace;
  font-size: 0.9rem; resize: none; outline: none; height: 80px; line-height: 1.4;
}
textarea::placeholder { color: rgba(207, 168, 99, 0.3); font-style: italic; }
textarea:disabled { opacity: 0.5; cursor: not-allowed; }

.btn-analyze {
  background: transparent; border: 1px solid var(--ink-white);
  color: var(--ink-white); font-family: 'Cinzel', serif; font-weight: 700;
  padding: 0 30px; cursor: pointer; text-transform: uppercase;
  transition: all 0.2s ease; margin-left: 10px; letter-spacing: 2px;
}
.btn-analyze:hover:not(:disabled) {
  background: var(--ink-white); color: black; box-shadow: 0 0 15px rgba(255,255,255,0.4);
}
.btn-analyze:disabled { opacity: 0.5; cursor: wait; }

.log-display {
  flex-grow: 1; font-size: 0.85rem; color: var(--faded-white);
  overflow-y: auto; height: 300px;
  scrollbar-width: thin; scrollbar-color: var(--faded-white) transparent;
}
...
.log-entry {
  margin-bottom: 8px; display: flex; opacity: 0; animation: fadeIn 0.3s forwards;
...
@keyframes textFlicker {
  0% { opacity: 0.9; } 3% { opacity: 0.4; } 6% { opacity: 0.9; }
  7% { opacity: 0.4; } 8% { opacity: 0.9; } 9% { opacity: 1; } 100% { opacity: 0.9; }
}
</style>